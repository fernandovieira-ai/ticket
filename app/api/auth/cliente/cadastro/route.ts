...
async function resolveEmpresaId(client: PoolClient): Promise<string | null> {
  if (process.env.EMPRESA_ID) return process.env.EMPRESA_ID;

  const res = await client.query(
    `SELECT id FROM empresas ORDER BY criado_em LIMIT 1`,
  );
...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success) {
    ...
    const campo =
      fieldLabels[firstError.path[0] as string] ?? String(firstError.path[0]);
