...
export async function POST(req: NextRequest) {
  ...
  if (usuario && usuario.perfil !== 'cliente') {
    // E-mail existe mas pertence a operador/admin/supervisor — informar painel correto
    return NextResponse.json(
      { error: 'Este e-mail pertence a uma conta de operador. Use o painel interno para recuperar sua senha.' },
      { status: 400 },
    )
  }

  if (!usuario) {
    console.log('[recuperar-senha] usuario nao encontrado para email:', email.toLowerCase())
  }
  ...
  if (usuario) {
    ...
    console.log('[recuperar-senha] resultado Resend:', JSON.stringify(resultado))
