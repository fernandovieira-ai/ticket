...
export async function GET(req: NextRequest) {
  ...
  if (grupoId) { conds.push(`r.grupo_id = $${p++}`); vals.push(grupoId); }
  if (status)  { conds.push(`r.status = $${p++}`);   vals.push(status); }
...
export async function POST(req: NextRequest) {
  ...
  if (!desde || !ate)
    return NextResponse.json({ error: "desde e ate são obrigatórios" }, { status: 400 });
  ...
  return NextResponse.json({ id: rel.id, status: "processando" }, { status: 202 });
}
