...
import { TrendingUp, RefreshCw, Loader2, Building2, User, ShieldCheck } from "lucide-react";
...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div style={{
      ...
      boxShadow: "0 2px 8px rgba(0,0,0,0.07)", minWidth: 200,
    }}>
      <p style={{ fontWeight: 600, color: "#18181B", marginBottom: 4 }}>{nomeCurto(d)}</p>
      <p style={{ color: "#71717A" }}>Número: <span style={{ color: "#3F3F46", fontWeight: 500 }}>+{d.numero}</span></p>
      {d.usuario_nome && (
        <p style={{ color: "#71717A" }}>
          Operador: <span style={{ color: "#6366F1", fontWeight: 600 }}>{d.usuario_nome}</span>
          {d.usuario_perfil && <span style={{ color: "#9CA3AF" }}> ({PERFIL_LABEL[d.usuario_perfil] ?? d.usuario_perfil})</span>}
        </p>
      )}
      <p style={{ color: "#71717A" }}>Mensagens: <span style={{ color: "#3F3F46", fontWeight: 600 }}>{d.total_mensagens}</span></p>
      <p style={{ color: "#71717A" }}>Grupos: <span style={{ color: "#3F3F46", fontWeight: 500 }}>{d.total_grupos}</span></p>
      {d.instancia_nome && <p style={{ color: "#6366F1", fontWeight: 500 }}>Instância do sistema</p>}
    </div>
  );
}
...
export default function RankingGruposPage() {
  ...
  const carregar = useCallback(async (dias: number) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ inicio: isoInicio(dias), fim: isoFim() });
  ...
  const operadores     = dados.filter((d) => !!d.usuario_id && !d.eh_instancia);

  const dadosFiltrados = dados.filter((d) => {
    if (filtroTipo === "operador")  return !!d.usuario_id && !d.eh_instancia;
    if (filtroTipo === "instancia") return d.eh_instancia;
    if (filtroTipo === "externo")   return !d.usuario_id && !d.eh_instancia;
    return true;
  });

  const chartData   = dadosFiltrados.map((d) => ({ ...d, nome_exibicao: nomeCurto(d).slice(0, 24) }));
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Quem mais enviou mensagens — vinculado ao usuário pelo telefone WhatsApp
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => carregar(periodo)} disabled={loading} className="gap-1.5">
      ...
      {!loading && dados.length > 0 && operadores.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-700 space-y-1">
          <p>
            Nenhum operador identificado. Copie o <strong>JID</strong> do participante na tabela abaixo
            e cole no campo <strong>WhatsApp JID</strong> de cada usuário em{" "}
            <a href="/painel/configuracoes/usuarios" className="underline font-medium">
              Configurações → Usuários
            </a>.
          </p>
          <p className="text-xs text-amber-600">
            Exemplo de JID: <code className="bg-amber-100 px-1 rounded">263689584308313@lid</code> ou{" "}
            <code className="bg-amber-100 px-1 rounded">5511999990000@s.whatsapp.net</code>
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-zinc-200 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-zinc-900 mt-1">{dadosFiltrados.length}</p>
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg p-4">
          <p className="text-xs text-zinc-500">Total de mensagens</p>
          <p className="text-2xl font-bold text-zinc-900 mt-1">{dadosFiltrados.reduce((s, d) => s + d.total_mensagens, 0)}</p>
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg p-4">
          <p className="text-xs text-zinc-500">Operadores identificados</p>
          <p className="text-2xl font-bold text-indigo-600 mt-1">{operadores.length}</p>
        </div>
      </div>

      {/* Gráfico */}
      <div className="bg-white border border-zinc-200 rounded-xl p-5">
        <p className="text-sm font-semibold text-zinc-700 mb-1">Mensagens por participante</p>
        <p className="text-xs text-zinc-400 mb-5">
          <span className="text-indigo-500 font-medium">Roxo</span> = operador/instância do sistema &nbsp;·&nbsp;
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={chartHeight}>
            <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 50, left: 0, bottom: 0 }} barSize={20}>
              <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#F4F4F5" />
              <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: "#9CA3AF" }} axisLine={false} tickLine={false} />
              <YAxis type="category" dataKey="nome_exibicao" width={160} tick={{ fontSize: 12, fill: "#374151" }} axisLine={false} tickLine={false} />
        ...
        <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-zinc-100">
            {dadosFiltrados.map((d, i) => (
              <div key={d.remetente_jid} className="flex items-center gap-3 px-5 py-3">
                <span className="text-xs font-bold text-zinc-400 w-5 text-right shrink-0">{i + 1}</span>
                <div
                  className="flex items-center justify-center rounded-full shrink-0"
                  style={{ width: 32, height: 32, backgroundColor: corBadge(d) }}
                >
                  {iconeRemetente(d)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-zinc-800 truncate">{nomeCurto(d)}</p>
                  <div className="flex items-center gap-2">
                    <p
                ...
                <div className="text-center shrink-0">
                  ...
                  <p className="text-sm font-semibold text-zinc-700">{d.total_grupos}</p>
                </div>
                <div className="text-right shrink-0 w-20">
                  <p className="text-xs text-zinc-400">Mensagens</p>
                  <p className="text-sm font-bold" style={{ color: (d.usuario_id || d.eh_instancia) ? "#6366F1" : "#16A34A" }}>
                    {d.total_mensagens}
                  </p>
                </div>
              </div>
